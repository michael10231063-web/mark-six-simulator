import { validHistoryDraw } from './draw-history.mjs';
export type Wins = [number, number, number, number, number, number, number];
export type Stats = { entries: number; cost: number; prize: number; unpricedEntries?: number; wins: Wins };
export type Ledger = { version: 2; legacy: Stats; draws: Record<string, Stats> };
export type SavedWin = { id: string; savedAt: string; drawNo: string; drawDate: string; numbers: number[]; extra: number; pick: number[]; tier: number; prize: number | null };
export function restoreWins(saved: string | null): SavedWin[] {
  ...
  if (!Array.isArray(value) || !value.every((w: SavedWin) => w && typeof w.id === 'string'
    ...
    && Number.isInteger(w.tier) && w.tier >= 0 && w.tier <= 6 && (w.prize === null || (Number.isFinite(w.prize) && w.prize >= 0)))) throw new Error('Invalid win collection');
  return value;
}
...
export function validStats(value: unknown): value is Stats {
  ...
  return [v.entries, v.cost, v.prize].every(n => typeof n === 'number' && Number.isFinite(n) && n >= 0)
    && (v.unpricedEntries === undefined || (Number.isSafeInteger(v.unpricedEntries) && v.unpricedEntries >= 0))
...
export function addStats(a: Stats, b: Stats): Stats {
  return { entries: a.entries + b.entries, cost: a.cost + b.cost, prize: a.prize + b.prize,
    ...((a.unpricedEntries ?? 0) + (b.unpricedEntries ?? 0) ? {unpricedEntries:(a.unpricedEntries ?? 0) + (b.unpricedEntries ?? 0)} : {}),
...
export function validDraw(value: unknown): boolean { return validHistoryDraw(value); }
